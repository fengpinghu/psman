# -*- coding: utf-8 -*-
"""ps takes a snapshot of the processes on a system and extract information  

It uses the ps command.
...
def walkto_root(pstable, exemptParentProcess, pid):
    ...
    if int(ps_ppid) < 2:
        ...
        ret = walkto_root(pstable, exemptParentProcess, ps_ppid)
        return ret
...
def is_exempted_ps(pstable, exeps, exeps_parent, pid):
    ...
    if ps_comm in exeps:
        exempted = True
    elif walkto_root(pstable, exeps_parent, pid):
        exempted = True
    return exempted
